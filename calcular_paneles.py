...
def calc_distribucion_prioridad_por_tipo(horizonte_dias: int = 30) -> list:
    """Distribución P1-P4 por tipo de vehículo para horizonte dado.
    
    Retorna lista de dicts: [{'tipo': 'Auto', 'prioridad': 'P1_critica', 'valor': 5}, ...]
    """
    result = query_db("""
        WITH ultimo_scoring AS (
            ...
            WHERE horizonte_dias = %(h)s
              AND prioridad IN ('P1_critica', 'P2_alta', 'P3_media', 'P4_baja')
        ...
        WHERE us.prioridad IS NOT NULL
        GROUP BY a.tipo_vehiculo, us.prioridad
        ORDER BY tipo, us.prioridad;
...
def calc_ranking_riesgo(horizonte_dias: int = 30, top_n: int = 15) -> list:
    """Calcula TOP N activos en riesgo para horizonte dado.
    
    Retorna lista de dicts con: activo_id, patente, marca, modelo, 
    prioridad, probabilidad_falla, fecha_scoring, sistema_riesgo, 
    dias_ultima_ot, km_actual, horizonte_dias, orden
    """
    
    # Definir cómo obtener sistema_riesgo según tablas disponibles
    ...
    result = query_db(q, params={"h": horizonte_dias, "n": top_n})
    
    if result is None or len(result) == 0:
        return []
    
    # Agregar orden (1-15) a cada registro
    registros = []
    for idx, row in enumerate(result, 1):
        row_dict = dict(row)
        row_dict['orden'] = idx
        row_dict['horizonte_dias'] = horizonte_dias
        registros.append(row_dict)
    
    return registros


def guardar_ranking_riesgo(registros: list, horizonte_dias: int) -> bool:
    """Guarda ranking precalculado en tabla paneles_ranking_riesgo.
    
    Elimina registros previos del mismo horizonte y inserta nuevos.
    """
    if not table_exists("paneles_ranking_riesgo"):
        log.warning("⚠ Tabla 'paneles_ranking_riesgo' no existe en Supabase")
        return False
    
    conn = get_connection()
    if conn is None:
        log.error("Guardar ranking: sin conexión a BD")
        return False
    
    try:
        with conn:
            with conn.cursor() as cur:
                # Limpiar registros anteriores de este horizonte
                cur.execute("DELETE FROM paneles_ranking_riesgo WHERE horizonte_dias = %s", (horizonte_dias,))
                
                # Insertar nuevos (solo si hay)
                if registros:
                    for reg in registros:
                        cur.execute("""
                            INSERT INTO paneles_ranking_riesgo 
                            (activo_id, patente, marca, modelo, prioridad, 
                             probabilidad_falla, fecha_scoring, sistema_riesgo, 
                             dias_ultima_ot, km_actual, horizonte_dias, orden, fecha_calculo)
                            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, CURRENT_TIMESTAMP)
                        """, (
                            reg['activo_id'], reg['patente'], reg['marca'], reg['modelo'],
                            reg['prioridad'], reg['probabilidad_falla'], reg['fecha_scoring'],
                            reg['sistema_riesgo'], reg['dias_ultima_ot'], reg['km_actual'],
                            horizonte_dias, reg['orden']
                        ))
        
        log.info(f"✓ Ranking riesgo (h={horizonte_dias}): {len(registros)} activos guardados")
        return True
    except Exception as e:
        log.error(f"Error guardando ranking riesgo: {str(e)[:100]}")
        return False
    finally:
        conn.close()
...
def calcular_todas_metricas():
    ...
    # ========== RANKING DE RIESGO (pre-calculado para dashboard) ==========
    log.info("\n📊 RANKING DE RIESGO (precalculado)")
    log.info("-" * 90)
    for horizonte in [7, 30, 90]:
        registros = calc_ranking_riesgo(horizonte_dias=horizonte, top_n=15)
        if guardar_ranking_riesgo(registros, horizonte):
            conteo['exitosas'] += 1
        else:
            conteo['fallidas'] += 1    
    
    # ========== DISTRIBUCIÓN PRIORIDAD POR TIPO ==========
    log.info("\n📊 DISTRIBUCIÓN PRIORIDAD POR TIPO")
    log.info("-" * 90)
    
    for horizonte in [7, 30, 90]:
        registros = calc_distribucion_prioridad_por_tipo(horizonte_dias=horizonte)
        for reg in registros:
            tipo = reg['tipo']
            prioridad = reg['prioridad']
            valor = reg['valor']
            
            metrica_compuesta = f"{prioridad} - {tipo}"
            valor_anterior = get_valor_anterior("Distribución Prioridad", metrica_compuesta, horizonte_dias=horizonte)
            
            if insert_metrica("Distribución Prioridad", metrica_compuesta, valor, valor_anterior, horizonte_dias=horizonte):
                conteo['exitosas'] += 1
            else:
                conteo['fallidas'] += 1
