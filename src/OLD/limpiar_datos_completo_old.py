...
def limpiar_datos_completo():
    ...
    3. ot_falla_evento (FK → ordenes_trabajo)
    4. ordenes_trabajo (FK → activos)
    5. activos (base)
    6. modelos_registro (independiente pero la limpias)
    7. audit_log (independiente)

    NO se trunca:
    - taxonomia_fallas: catálogo persistente de referencia. Es padre de
      ot_falla_evento vía la FK obligatoria taxonomia_id. Truncarlo rompería
      la integridad referencial y obligaría a re-sembrar el catálogo.
    ...
    try:
        ...
        # 3. OT_FALLA_EVENTO (depende de ORDENES_TRABAJO)
        conn.execute(text("TRUNCATE TABLE ot_falla_evento CASCADE;"))
        print("✅ ot_falla_evento truncada")

        # 4. ORDENES_TRABAJO (depende de ACTIVOS)
        conn.execute(text("TRUNCATE TABLE ordenes_trabajo CASCADE;"))
        print("✅ ordenes_trabajo truncada")

        # 5. ACTIVOS (tabla base)
        conn.execute(text("TRUNCATE TABLE activos CASCADE;"))
        print("✅ activos truncada")

        # 6. MODELOS_REGISTRO (independiente)
